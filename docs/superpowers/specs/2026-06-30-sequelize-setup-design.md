...
5. `await runMigrations()` — umzug runs `up()` on any pending migration in every environment. Matches the "always run pending migrations at boot" behavior the user picked (eager boot, no environment guard).
...
The pre-existing `"test": ""` placeholder is replaced.
